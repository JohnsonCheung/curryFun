...
export const sCmp = (exp: s, act: s): void => {
    sBrwAtFdrFn('strCmp', 'exp')(exp)
    sBrwAtFdrFn('strCmp', 'act')(act)
    debugger
}
export const vCmp = (exp, act): void => {
    if (isBool(exp) && isBool(act)) {
        if (act !== exp) {
            debugger
            return
        }
    }
    if (isStr(exp) && isStr(act))
        return sCmp(exp, act)
    oBrwAtFdrFn('vCmp', 'exp')(exp)
    oBrwAtFdrFn('vCmp', 'act')(act)
    debugger
}
export const assertIsEq = (exp, act) => isNotEq(exp, act) ? vCmp(exp, act) : void 0;
export const assertIsNotEq = (exp, act) => isEq(exp, act) ? vCmp(exp, act) : void 0;
//-------------------------------------------------------------------------
...
export const sRmvSfx_IGNORE_CASE = (sfx: s) => (a: s) => sHasSfx(sfx)(a) ? a.substr(0, a.length - sfx.length) : a

export const sHasPfxIgnCas = sHasPfx_IGNORE_CASE
...
export const sPthSepPosRev = (s: s) => {
    const z = s.lastIndexOf('\\')
    if (z >= 0)
        return z
    return s.lastIndexOf('/')
}
export const pthPar = (a: pth): pth => {
    const segAy = pthSegAy(a)
    segAy.pop()
    segAy.pop()
    return segAy.join(pthsep) + pthsep
}
export const pthSegAy = (a: pth): seg[] => a.split(/[\\\/]/g)
export const pthBrw = (a: pth) => cmdShell(sFmt('explorer "?"', a))
export const ffnPth = (a: ffn) => { const at = sPthSepPosRev(a); return at === -1 ? '' : sLeft(at + 1)(a) }
export const ffnFn = (a: ffn) => { const at = sPthSepPosRev(a); return at === -1 ? a : sMid(at + 1)(a) }
...
export const isFfnExist = (a: ffn) => fs.existsSync(a)
...
export const isFTstJs = (_fTstJs: fTstJs) => {
    const fn = ffnFn(_fTstJs)
    if (!sHasPfx('tst__')(fn))
        return false
    if (!sHasSfx('.js')(fn))
        return false
    const segAy = ffnPth(_fTstJs).split(/[\\\/]/)
    segAy.pop()
...
export const assertIsTrue = (v, ...msg) => {
    if (v) return
    if (msg.length === 0)
        er('given vaule should be true')
    else {
        const m = msg.shift()
        er(m, msg)
    }
}
export const assertIsFTstJs = (_fTstJs: fTstJs) => assertIsTrue(isFTstJs(_fTstJs), "given _fTstJs is not fTstJs", { _fTstJs })
export const fTstJs_fts = (_fTstJs: fTstJs): fts => {
    assertIsFTstJs(_fTstJs)
    const fn = ffnFn(_fTstJs)
    const pth = ffnPth(_fTstJs)
    const a1 = sRmvPfx('tst__')(fn)
    const a2 = aa
    const zFnn = ffnFnn(fn)
    const zPth = pthPar(pth)
    return zPth + zFnn + '.js'
}
...
export const max = (...v) => itrMax(v)
export const min = (...v) => itrMin(v)
//-----------------------------------------------------------------------------------------
...
export const nmPrm_ny = (_nmPrm: nyPrm): ny => {
    if (typeof _nmPrm === 'string')
        return sSplitSpc(_nmPrm)
    return _nmPrm
}
export const ny = nmPrm_ny
export const oPrpAy = (_prpNm: nyPrm) => (_o: o) => itrMap((nm: s) => oPrp(nm)(_o))(ny(_prpNm))
...
const drySrtCol__srtFun = (colAy: n[]) => (drA: ay, drB: ay) => {
...
export const drySrtCol = (colAy: n[]) => (a: dry) => a.sort(drySrtCol__srtFun(colAy))
...
export type lazy<T> = () => { v: T }
export const lazy = <T>(vf: (() => T)): lazy<T> => { let v, done = false; return () => { if (!done) { v = vf(); done = true }; return v } }
//---------------------------------------------------------------------------
...
export const vidpth = __dirname + pthsep + 'test-resources' + pthsep
pthEns(vidpth)
export const rePunExcpDot = /[\(\)]/g
export const sRplPunExcpDot = (s: s): s => s.replace(rePunExcpDot, ' ')
...
export const vidBrw = (a: vid) => ftBrw(vidFjson(a))
export const sSav = (sid: sid) => (a: s) => sWrt(sidFt(sid))(a)
export const sidpth = vidpth
export const sidpthBrw = () => pthBrw(sidpth)
export const sidFt = (a: sid) => sidpth + a + '.txt'
export const sidStr = (a: sid) => ftLines(sidFt(a))
export const sidBrw = (a: sid) => ftBrw(sidFt(a))
...
export const cmdShellSync = child_process.exec as (a: s) => void
export const ftBrw = (a: ft) => cmdShell(`code.cmd "${a}"`)
export const ftBrwSync = (a: ft) => cmdShellSync(`code.cmd "${a}"'`)
...
export const srcExpConstNyBrw = compose(srcExpConstNy, nyBrw) as (_src: src) => void
export const ftsExpConstNyBrw = compose(ftLy, srcExpConstNyBrw)
export const oBrw = (o, fdr?: s, nm?: s): void => {
    const s = oJsonLines(o)
    sjsonBrw(s, fdr, nm)
}
...
export const drsof_exportFunctions = () => {
    ...
    let dry: dry = []
    let md: NodeModule
    const mset = nodeMdSet()
    for (md of mset) {
        dry = dry.concat(x(md))
    }
    dry = drySrtCol([2, 0])(dry)
    const z: drs = { fny, dry }
    return z
}
export class Dry {
