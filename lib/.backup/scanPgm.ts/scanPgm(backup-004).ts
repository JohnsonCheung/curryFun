...
const xn2_part2 = (ly: ly, ix1: n, ix2: n): lines => {
    ...
    //    const funNy = xn2n_srtedFunNy(ly)
...
const xn2tn_nLvl = (ny: ny) => cf.itrMax(cf.itrMap(y_nm_lvlI)(ny))
...
const xn2tl_lvlINy = (lvlI: n, ny: ny): ny => cf.itrWhere(xn2tli_isLvlINm(lvlI))(ny).sort()
...
const xn2t_srtedTstFunNy = (_ly: ly): nm[] => {
    ...
    const n0 = xn2tn_nLvl(ny)
    const n = cf.vDft(0)(n0)
    let o: ny = []
    for (let lvlI = -1; n > lvlI; lvlI++) {
        o = o.concat(xn2tl_lvlINy(lvlI, ny))
    }
...
const xn2n_srtedFunNy = (_ly: ly): nm[] => {
...
function tst__xn2tn_nLvl() {
    function r(exp: n, ny: ny) {
        const act = xn2tn_nLvl(ny)
