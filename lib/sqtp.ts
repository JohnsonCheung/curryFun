...
function tst__xwww_sw() {
    ...
    assertIsEq(fldSw, new Map<s, b>([['b', false]]))
    assertIsEq(stmtSw, new Map<s, b>([['?#', true]]))
}
...
if (module.id === '.') {
    ...
    /*
        ...
        */
    //    tst__sqtpRslt()
    dic_dftVal                        // = <T>(dft: T) => (dic: dic<T>, key: s): T => {
    lin_AddMrk                        // = (a: lin, pos: n, len: n): lin => {
    lin_fmT3DupTermMrkrLin            // = (a: lin): lin => {
    lin_fmT3DupTermSet                // = (_lin: lin): sset => {
    lin_t1MrkrLin                     // = (a: lin, msg: s) => {
    lin_t2MrkrLin                     // = (a: lin, msg: s) => {
    lin_t2PosWdt                      // = (a: lin): posWdt | null => {
    lin_termAy                        // = (_lin: lin): term[] => _lin.trim().split(/\s+/)
    lin_termPosWdtAy                  // = (a: lin): posWdt[] => {
    ly_fstTermAy                      // = x.itrMap(x.linFstTerm) as (ly: ly) => ly
    ly_fstTermDupSet                  // = x.compose(ly_fstTermAy, x.itrDupSet) as (ly: ly) => sset
    ly_sdic                           // = (a: ly) => {
    posLin_ParseSpc                   // = ({ pos, lin }: posLin): posLin => {
    posLin_ParseTerm                  // = ({ pos, lin }: posLin): [term, posLin] => {
    sq_AND                            // = 'AND'
    sq_DIS                            // = 'DIS'
    sq_DRP                            // = 'DRP'
    sq_FRO                            // = 'FRO'
    ...
    sq_UPD                            // = 'UPD'
    sq_WHE                            // = 'WHE'
    sqtpRslt                          // = (_sqtp: sqtp): { vtp: s, sql: s } => {
